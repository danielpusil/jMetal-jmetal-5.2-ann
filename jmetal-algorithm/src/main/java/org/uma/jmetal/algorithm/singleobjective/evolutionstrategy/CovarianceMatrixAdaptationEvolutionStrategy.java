...
public class CovarianceMatrixAdaptationEvolutionStrategy
    ...
    public static class Builder {
        ...
        private static final int DEFAULT_MAX_EVALUATIONS = 300;
        ...
        public Builder(DoubleProblem problem, int lambda, double sigma, int maxEvaluations) {
            this.problem = problem;
            this.lambda = lambda;
    ...
    private int updateEvolutionPaths(double[] oldDistributionMean) {
        ...
        // cumulation for covariance matrix (pathsC)//check ratio
        int hsig = 0;
    ...
    private DoubleSolution sampleSolution() {
        ...
        for (int i = 0; i < numberOfVariables; i++) {
            ...
            artmp[i] = diagD[i] * distributionRnd.nextGaussian();
        }
        for (int i = 0; i < numberOfVariables; i++) {
            sum = 0.0;
            for (int j = 0; j < numberOfVariables; j++) {
    ...
    private void storeBest() {
        if ((bestSolutionEver == null) || comparator.compare(bestSolutionEver, getPopulation().get(0)) == 1) {
